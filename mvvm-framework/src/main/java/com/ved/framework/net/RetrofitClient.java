...
//                        .cookieJar(new CookieJarImpl(new PersistentCookieStore(Utils.getContext())))
                        ...
                        .addInterceptor(chain -> {
                            ...
                            try {
                                ...
                                Result r = JsonPraise.jsonToObj(datas, Result.class);
                                String message = "";
                                EntityResponse entityResponse = JsonPraise.jsonToObj(content,EntityResponse.class);
                                if (entityResponse != null) {
                                    message = (String) entityResponse.getData();
                                }
                                if (r != null) {
                                    code = r.getResult();
                                }
    ...
    public <T> T create(final Class<T> service, String baseUrl, Map<String, String> headers, IResult iResult) {
        if (service == null) {
            throw new RuntimeException("Api service is null!");
        }
        HttpsUtils.SSLParams sslParams = HttpsUtils.getSslSocketFactory();
...
//                        .cookieJar(new CookieJarImpl(new PersistentCookieStore(Utils.getContext())))
                        ...
                        .addInterceptor(chain -> {
                            ...
                            {
                                e.printStackTrace();
                                SPUtils.getInstance().putString("document",content);
                                iResult.onInfoResult(content,1);
                            }
                            return response.newBuilder().body(okhttp3.ResponseBody.create(mediaType, content)).build();
                        }).addInterceptor(new HttpLoggingInterceptor().setLevel(HttpLoggingInterceptor.Level.HEADERS))
                        .connectTimeout(Constant.DEFAULT_TIMEOUT, TimeUnit.SECONDS)
